...
uint64_t vmm_space_fork(uint64_t src_cr3) {
    uint64_t new_cr3 = vmm_space_clone();
    if (new_cr3 == 0) {
        return 0;
    }

    uint64_t *src_pml4 = (uint64_t *)pmm_phys_to_virt(src_cr3);
    for (int i4 = 0; i4 < 256; i4++) { /* user half only */
        if (!(src_pml4[i4] & VMM_PRESENT) || (src_pml4[i4] & (1ull << 7))) {
            continue; /* not present, or a huge page - see vmm_space_fork's declaration */
        }
        uint64_t *pdpt = (uint64_t *)pmm_phys_to_virt(src_pml4[i4] & PTE_ADDR_MASK);

        for (int i3 = 0; i3 < 512; i3++) {
            if (!(pdpt[i3] & VMM_PRESENT) || (pdpt[i3] & (1ull << 7))) {
                continue;
            }
            uint64_t *pd = (uint64_t *)pmm_phys_to_virt(pdpt[i3] & PTE_ADDR_MASK);

            for (int i2 = 0; i2 < 512; i2++) {
                if (!(pd[i2] & VMM_PRESENT) || (pd[i2] & (1ull << 7))) {
                    continue;
                }
                uint64_t *pt = (uint64_t *)pmm_phys_to_virt(pd[i2] & PTE_ADDR_MASK);
                ...
                for (int i1 = 0; i1 < 512; i1++) {
                    ...
                    uint64_t src_phys = pt[i1] & PTE_ADDR_MASK;
                    uint64_t flags = pt[i1] & 0xFFFull;

                    uint64_t new_phys = pmm_alloc_frame();
                    if (new_phys == 0) {
                        return 0; /* caller frees the kstack it already
                                   * allocated; the partial space is
                                   * simply never switched to */
                    }
                    memcpy((void *)pmm_phys_to_virt(new_phys),
                          (void *)pmm_phys_to_virt(src_phys), PAGE_SIZE);
                    if (vmm_map_page_in(new_cr3, virt, new_phys, flags) != 0) {
                        return 0;
                    }
                }
            }
...
/* Bump allocator over the MMIO reservation. Device register blocks
 * are mapped once, at boot, and never unmapped, so there is nothing
 * for a free list to do. */
static uint64_t g_mmio_next = VMM_MMIO_BASE;
...
void *vmm_map_mmio(uint64_t phys, size_t bytes) {
    ...
    uint64_t base = phys - page_off;
    size_t span = (bytes + page_off + PAGE_SIZE - 1) & ~(uint64_t)(PAGE_SIZE - 1);

    if (g_mmio_next + span > VMM_MMIO_BASE + VMM_MMIO_SIZE) {
        return NULL;
    }
    uint64_t virt = g_mmio_next;
    if (vmm_map_region(virt, base, span,
                       VMM_PRESENT | VMM_WRITE | VMM_NOCACHE) != 0) {
        return NULL;
    }
    g_mmio_next += span;
