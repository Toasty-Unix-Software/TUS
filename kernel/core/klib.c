 ...
 *
 * They deliberately stay byte-granular. Moving eight bytes per
 * iteration is eight times fewer operations under emulation and is
 * measurably faster, but making every copy in the kernel that much
 * quicker reproducibly panicked the display server's spawn path -
 * a ring-3 page fault with a kernel pointer in a user program - while
 * a self-test proved the wide routines themselves correct at every
 * size and alignment. Something in that path depends on timing;
 * until it is found, this stays as it is. The console's own scroll,
 * which is the one copy big enough to feel, does its own wide move in
